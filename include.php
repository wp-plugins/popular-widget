...
class PopularWidgetFunctions {
	...
	*/
	function get_comments($instance){
		...
		$comments = wp_cache_get("pop_comments_{$time}",'pop_cache');
		if($comments == false) {
		...
		foreach($comments as $comment){
			...
			$output .= ($image) ? $image.'<span class="pop-overlay">':'<span class="pop-text">';
	...
	*/
	function get_most_viewed( $instance ){
		...
		if($viewed == false) {
			...
			wp_cache_set("pop_viewed_{$time}",$viewed,'pop_cache');
		}
